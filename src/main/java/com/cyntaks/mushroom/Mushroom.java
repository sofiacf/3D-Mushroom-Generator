...
public class Mushroom {
	...
	private final int TRIANGLES_IN_RING;
	private final int POINTS_IN_RING;
	private final int STEM_RINGS;
	private final float RING_HEIGHT;
	...
	private final int HEAD_SECTORS;
	private final int TRIANGLES_IN_HEAD_SECTOR;
	private final int POINTS_IN_HEAD_SECTOR;
	...
	private final int normalVBOID2;
	...
	private float[][] createStem() {
		...
		for (int i = 0; i < STEM_RINGS; i++) {
			...
			System.arraycopy(ringPoints, 0, stemPoints, i * POINTS_IN_RING + 0, ringPoints.length);
		}
	...
	private void setupRenderingAttributes()
	{
		...
		else RenderState.disableLighting();

		if (this.useBlending)
			RenderState.enableBlending();
		else RenderState.disableBlending();

		if (this.useBlending)
			RenderState.setBlendingMode(this.blendMode);

		RenderState.setPolygonRenderingMode(this.renderBackfaces, this.polygonRenderMode);
	}
...
//		GL11.glDisable(GL11.GL_LIGHTING);
		...
		GL20.glUniform1f(lightTimerLoc, Generator.lightTimer);
	...
	private void fillNormalBuffer2() {
		...
		for (int i = 0; i < max; i++) {
			...
			float[] point2 = interpolate(stemPoints[i+1], nextStemPoints[i+1], timeRatio);
			float[] point3 = interpolate(stemPoints[i+2], nextStemPoints[i+2], timeRatio);
			float[] normal = Generator.getNormal(point, point2, point3);
